...
for snr = -3:-6:-9
    ...
    for omegaw = 1:omegawmax
        ...
        % Lawan's algorithms select two max-gap codebooks from the same
        % feasible set: backward max-gap first and forward max-gap second.
...
%%%%        greedy = codeset(feasible,w);
        maxgap = maxgapcodeset(feasible,w);
...
%%%%        plotset(omegaw,:) = [omegaw,min(diff(greedy,1,1), [], 1)];  % 1-by-numCols
        plotset(omegaw,:) = [omegaw,min(diff(maxgap,1,1), [], 1)];  % 1-by-numCols
...
%%%%        plotset(count,:) = [fmax,min(diff(greedy,1,1), [], 1)];  % 1-by-numCols
        plotset(count,:) = [fmax,min(diff(maxgap,1,1), [], 1)];  % 1-by-numCols
        count = count+1;
...
function maxgap = maxgapcodeset(feasible,w)
...
% Return NaN values so the parameter sweep and plots can continue.
if length(feasible) < esize
    warning('Only %d feasible rows found; need %d. Plot values set to NaN.', ...
...
while low <= high
    ...
    chosenIdx = greedyChooseWithGapBackward( ...
...
% Sort the selected rows in increasing sequency order for plotting.
...
while low <= high
    ...
    chosenIdx = greedyChooseWithGapForward( ...
        faithfulIdxSorted,faithfulFreqSorted,K,gap);
